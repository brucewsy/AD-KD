...
def distill_train(args, train_dataset, dev_dataset, student, teacher, tokenizer, logger):
    ...
    if "saliency" in args.distill_loss.split('+'):
        if args.task_name=='mnli':
            saliency_function = saliency_mse_for_multiclass(top_k=args.topk, norm=args.normalization_type, loss_func=args.loss_func).to(args.device)
        else:
            saliency_function = saliency_mse(top_k=args.topk, norm=args.normalization_type, loss_func=args.loss_func).to(args.device)
